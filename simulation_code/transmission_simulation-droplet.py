...
class EpidemicDisease(object):
    ...
    def __init__(
        ...
        self.beta_max = self.beta_0 / (self.rho_daily * self.sigma_r ** 2 * self.sigma_theta ** 2)
...
class Classroom(object):
    def __init__(
            ...
            half_class: bool, vaccinated: bool, vaccine_efficacy_rate: float, output_interval: int, record_beta: bool
    ):
    ...
    def _append_status(output_path: Path, status):
        with open(str(output_path), 'a') as f:
            f.write('{}\n'.format(','.join(str(int(status[sub])) for sub in sorted(status))))
    ...
    def simulate_transmission(self, t, status, loc_dict, progress_time, rm: random.Random, beta_path: Path):
        ...
        if self.record_beta:
            with open(str(beta_path), 'a') as f:
                f.write('{}\n'.format(','.join('{:.5e}'.format(sum_beta[sub]) if status[sub] == Status.HEALTH else 'na' for sub in sorted(sum_beta))))

        for kid in status:
            if status[kid] == Status.HEALTH:
                if rm.uniform(0.0, 1.0) <= sum_beta[kid]:
    ...
    def simulate(self, status, out_path: Path, rm: random.Random):
        ...
        if self.record_beta:
            beta_path = out_path.parent / out_path.name.replace('simulation', 'beta')
            ...
            for loc_dict in self.location_data:
                if t % self.output_interval == 0:
                    ...
                    if Status.INFECTED not in status.values():
                        return
                status, progress_time = self.simulate_transmission(t, status, loc_dict, progress_time, rm, beta_path)
                t += 1
    ...
    def run_simulation(self, idx):
        ...
        output_folder = self.output_root / '{}{}'.format('half_class' if self.half_class else 'full_class', '_vaccinated' if self.vaccinated else '') / 'zero_patient_{}'.format(zero_patient_index)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Epidemic Disease Transmission Simulation in the Classroom')
    parser.add_argument('data_path', type=str, help='Path to the data folder of the classroom observation')
    parser.add_argument('num_simulations', type=int, help='Number of simulations per observation per zero patient')
    ...
    parser.add_argument('--id_offset', type=int, default=0, help='The offset of simulation id')
    parser.add_argument('--record_beta', action='store_true', help='Determine whether to record beta or not')
    ...
    cls = Classroom(
        ...
        Path(args.data_path) / args.ob_data, Path(args.data_path) / args.output_root, args.max_simulation_day,
        args.half_class, args.vaccinated, args.vaccine_efficacy_rate, args.output_interval, args.record_beta
    )
    with concurrent.futures.ProcessPoolExecutor() as executor:
        if args.vaccinated:
            results = executor.map(
                cls.run_simulation,
                itertools.product(cls.kid_indices(), list(np.arange(args.num_simulations) + args.id_offset))
            )
        else:
            results = executor.map(
                cls.run_simulation,
                itertools.product(cls.sub_indices(), list(np.arange(args.num_simulations) + args.id_offset))
            )
